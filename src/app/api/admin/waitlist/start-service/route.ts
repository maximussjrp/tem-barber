...
export async function POST(request: NextRequest) {
  ...
  if (typeof body.entryId !== "string" || typeof body.memberId !== "string") {
    return NextResponse.json({ error: "entryId e memberId são obrigatórios." }, { status: 400 });
  }
  try {
    const result = await startCalledWaitlistEntry({
      ...
      startedByUserId: auth.data.userId,
    });
