    ...
    async function fetchData() {
      ...
      try {
        ...
        if (!ignore) {
          ...
          setPlans(Array.isArray(pd) ? pd.filter((p: { isActive?: boolean }) => p.isActive) : []);
        }
      } catch {
        if (!ignore) setError("Erro ao carregar assinantes.");
      } finally {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      <ClubModal
        ...
        description={detailSub ? `Plano: ${detailSub.clubPlan.name}` : undefined}
        footer={
          <>
            ...
            <button onClick={handlePay} disabled={payLoading} className="btn-gold px-5 py-2 text-sm min-h-0 disabled:opacity-50">
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className={labelCls}>Valor mensal do plano (R$)</label>
            <input className={inputCls} type="text" readOnly value={detailSub ? brl(detailSub.clubPlan.monthlyPrice) : ""} disabled />
          </div>
          <div>
          ...
          <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-3 text-xs space-y-1 text-[var(--text-muted)]">
            <p className="font-semibold text-[var(--text-primary)]">Novo ciclo gerado automaticamente:</p>
            <p>• 30 dias de vigência a partir do início do período</p>
            <p>• 1 dia de carência adicional após a data limite de vigência</p>
          </div>
        </div>
      </ClubModal>
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <ClubStatusBadge status={effStatus} type="subscription" />
                    </div>
