...
describe("Plan Code Resolution & Commercial Consistency (D2A)", () => {
  ...
  it("getPlanByCode resolves plan by code regardless of isActive status", async () => {
    const mockDb = {
      plan: {
  ...
  it("getActivePlanByCode throws PLAN_CODE_INACTIVE if plan exists but isActive=false", async () => {
    const mockDb = {
      plan: {
        findFirst: vi.fn().mockResolvedValue(mockDbPlanInactive),
      },
    };
  ...
  it("assertCommercialConsistency passes when price and period match", () => {
